...
def finalize_db(db_path: str | Path, drop_raw_text: bool = False) -> None:
    """Materialise derived columns the Streamlit app reads on every request.

    Adds two columns to ``speeches`` and fills them once:
      * ``faction_normalized`` — the resolved party (see FACTION_NORMALIZE_SQL);
        removes an expensive per-query CASE/sub-select from the read path.
      * ``search_text`` — ``lower(speech_content)``; substring search then scans
        a pre-lowered column instead of calling ``lower()`` over ~1.8 GB of text
        on every query (~5× faster in practice).

    Run after the speeches AND ministers tables are loaded (the faction
    fallback references ministers).  Safe to re-run — columns are refreshed.

    Set ``drop_raw_text=True`` to drop the original ``speech_content`` after
    building ``search_text``.  The app never displays raw text, so this roughly
    halves the database size for a leaner deploy artifact — but it is
    destructive and the text can only be recovered by re-running the pipeline.
    """
    with duckdb.connect(str(db_path)) as conn:
        ...
        if drop_raw_text:
            ...
            print("[finalize] dropped raw speech_content", flush=True)
