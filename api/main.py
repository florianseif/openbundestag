...
@app.get("/health")
def health() -> dict:
    has_text = q.has_raw_text(_con.cursor()) if _con is not None else False
    return {"status": "ok" if _con is not None else "starting", "has_raw_text": has_text}
