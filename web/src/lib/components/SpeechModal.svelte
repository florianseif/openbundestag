<script lang="ts">
	...
	function download(item: SpeechItem) {
		const full = fullCache[item.id];
		const body = full?.speech_content ?? item.snippet ?? '';
		const lines = [
			`${item.politician} (${item.party})`,
			`${formatDate(item.date, i18n.lang)} · ${i18n.t('term_short')} ${item.electoral_term} · ${i18n.t('session')} ${item.session}`,
			item.position_long ?? item.position_short,
			'',
			body || i18n.t('fulltext_unavailable'),
			'',
			'— Deutscher Bundestag · Plenarprotokolle (bundestag.de/services/opendata) · via OpenBundestag'
		].join('\n');
