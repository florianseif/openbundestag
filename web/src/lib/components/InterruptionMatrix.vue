...
import { partyColor, partyFoundingOrder, formatNumber } from '$lib/format';
...
// Reactive defaults (Svelte runes prop defaults re-evaluate on language toggle;
// withDefaults() would freeze them, so fall back via computed instead).
const showSamples = computed(() => props.showSamples ?? false);
const axisRow = computed(() => props.axisRow ?? i18n.t('mx_axis_calling'));
const axisCol = computed(() => props.axisCol ?? i18n.t('mx_axis_interrupted'));
const noun = computed(() => props.noun ?? i18n.t('mx_noun_zw'));

// Derive ordered party lists from data
const callers = computed(() =>
	[...new Set(props.rows.map((r) => r.caller_party))].sort(
		(a, b) => partyFoundingOrder(a) - partyFoundingOrder(b)
	)
);
const targets = computed(() =>
	[...new Set(props.rows.map((r) => r.target_speaker_party))].sort(
		(a, b) => partyFoundingOrder(a) - partyFoundingOrder(b)
	)
);
...
const lookup = computed(
	() => new Map(props.rows.map((r) => [`${r.caller_party}→${r.target_speaker_party}`, r.n]))
);
const maxVal = computed(() => Math.max(1, ...props.rows.map((r) => r.n)));

function cell(caller: string, target: string): number {
	return lookup.value.get(`${caller}→${target}`) ?? 0;
}

// Intensity mapped to alpha: 0 → 0.04, max → 0.88
function intensity(n: number): number {
	return n === 0 ? 0 : 0.08 + (n / maxVal.value) * 0.8;
}
...
// Precompute the cell grid (replaces Svelte's per-iteration {@const}).
const matrix = computed(() =>
	callers.value.map((caller) => ({
		caller,
		cells: targets.value.map((target) => {
			const n = cell(caller, target);
			return { target, n, alpha: intensity(n), isSelf: caller === target };
		})
	}))
);

const hoveredCell = ref<{ caller: string; target: string; n: number } | null>(null);
const samples = ref<ZwischenrufSample[]>([]);
const samplesLoading = ref(false);
let samplesDebounce: ReturnType<typeof setTimeout>;

function fetchSamples(caller: string, target: string) {
	if (!showSamples.value) return;
	clearTimeout(samplesDebounce);
	samples.value = [];
	samplesDebounce = setTimeout(async () => {
		samplesLoading.value = true;
		try {
			samples.value = await api.zwischenrufe.samples({
				callerParty: caller,
				targetParty: target,
				limit: 5
			});
...
function onEnter(caller: string, target: string, n: number) {
	hoveredCell.value = n > 0 ? { caller, target, n } : null;
...
<template>
	<div class="matrix-wrap">
		<div class="grid" :style="{ '--cols': targets.length + 1, '--rows': callers.length + 1 }">
			<!-- Top-left corner -->
			<div class="corner">
			...
			<template v-for="target in targets" :key="'h-' + target">
			...
			<template v-for="row in matrix" :key="row.caller">
				<!-- Row header -->
				<div class="header row-header" :title="row.caller">
					<span class="dot" :style="{ background: partyColor(row.caller) }"></span>
					{{ abbrev(row.caller) }}
				</div>

				<!-- Data cells -->
				<template v-for="c in row.cells" :key="c.target">
					<!-- role + tabindex are both gated on n > 0, so a focusable cell is always a button -->
					<div
						class="cell"
						:class="{ self: c.isSelf, active: c.n > 0 }"
						:style="{ '--alpha': c.alpha, '--cc': partyColor(row.caller) }"
						:role="c.n > 0 ? 'button' : 'presentation'"
						:tabindex="c.n > 0 ? 0 : undefined"
						:aria-label="
							c.n > 0
								? `${row.caller} → ${c.target}: ${formatNumber(c.n, i18n.lang)} ${noun}`
								: undefined
						"
						@mouseenter="onEnter(row.caller, c.target, c.n)"
						@mouseleave="onLeave"
						@focus="onEnter(row.caller, c.target, c.n)"
						@blur="onLeave"
					>
						<span v-if="c.n > 0" class="n">{{ formatNumber(c.n, i18n.lang) }}</span>
					</div>
				</template>
			</template>
		</div>
		...
		<div class="mx-foot">
			...
			<span class="mx-diag-note"
				><span class="mx-diag-swatch" aria-hidden="true"></span>{{ i18n.t('mx_diagonal') }}</span
			>
		</div>

		<!-- Hover panel -->
		<div v-if="hoveredCell" class="tooltip" :class="{ 'has-samples': showSamples }">
			<div class="tt-header">
			...
			<div v-if="showSamples" class="tt-samples">
				<span v-if="samplesLoading" class="tt-loading">…</span>
				<template v-else-if="samples.length">
					<div v-for="(s, i) in samples" :key="i" class="tt-sample">
