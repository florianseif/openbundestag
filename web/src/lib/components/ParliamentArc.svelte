<script lang="ts">
	...
	const active = $derived(hovered ? sectors.find((s) => s.party === hovered) ?? null : null);
	const pct = (share: number) => {
		const v = (share * 100).toFixed(1);
		return i18n.lang === 'de' ? v.replace('.', ',') : v;
	};
	const activeLabel = $derived(
		active ? (active.other ? i18n.t('arc_other') : active.party) : null
	);
</script>
...
>
	...
	<g filter="url(#parl-glow)" class="dots" class:focused={hovered}>
		{#each dots as d}
			<circle
				...
				fill={d.color}
				class="d r{d.row}"
				class:dim={hovered && d.party !== hovered}
			/>
		{/each}
	</g>
	...
	{#each sectors as s (s.party)}
		<path
			d={wedgePath(s.a0, s.a1)}
			class="wedge"
			role="presentation"
			onmouseenter={() => (hovered = s.party)}
		/>
	{/each}
</svg>
