...
// Full port of +layout.svelte. app.css is imported in main.ts.
//   $app/state `page` -> vue-router useRoute()
//   $derived(...)      -> computed(...)
//   {@render children()} -> <RouterView />
//   nav <a href> -> <RouterLink to> (SPA navigation)
import { computed } from 'vue';
import { useRoute, RouterLink, RouterView } from 'vue-router';
import LangToggle from '$lib/components/LangToggle.vue';
import { i18n } from '$lib/i18n';

const route = useRoute();
const onWortsuche = computed(() => route.path.startsWith('/explore'));
const onZwischenrufe = computed(() => route.path.startsWith('/zwischenrufe'));
const onBeifall = computed(() => route.path.startsWith('/beifall'));
const onAbout = computed(() => route.path.startsWith('/about'));
</script>
	...
	<footer class="site">
		<div class="wrap">
			...
			<p class="ft-attr ft-data">
				<a
					href="https://huggingface.co/datasets/MissionJupiter/openbundestag-db"
					target="_blank"
				...
				·
				<a
					href="https://github.com/florianseif/openbundestag"
					target="_blank"
					rel="noopener noreferrer"
				>
					GitHub ↗
