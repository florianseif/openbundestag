...
async function boot() {
	...
	loading.value = true;
	try {
		meta.value = await api.beifall.meta();
	} catch {
...
async function reload() {
	...
	if (svo.status === 'fulfilled') selfVsOther.value = svo.value;
}
...
const selfVsOtherBars = computed(() => {
	const byPartyMap = new Map<string, { self: number; other: number }>();
	for (const row of selfVsOther.value) {
...
// Stat card values
const topApplauder = computed(() => visibleByParty.value[0] ?? null);
const mostApplauded = computed(() => {
...
const mostSelfApplause = computed(
	() => [...selfVsOtherBars.value].sort((a, b) => b.selfPct - a.selfPct)[0] ?? null
);

watchEffect(() => {
	document.title = `${i18n.t('bf_title')} · OpenBundestag`;
});
</script>
		...
		<template v-else>
			...
			<div class="metrics">
				...
				<div
					v-if="topApplauder"
					class="metric glass"
					:style="{ '--lc': partyColor(topApplauder.caller_party) }"
				>
				...
				<div
					v-if="mostApplauded"
					class="metric glass"
					:style="{ '--lc': partyColor(mostApplauded.party) }"
				>
					<span class="m-num small lead">
						<span class="lead-dot"></span>
						{{ mostApplauded.party }}
				...
				<div
					v-if="mostSelfApplause"
					class="metric glass"
					:style="{ '--lc': partyColor(mostSelfApplause.party) }"
				>
					<span class="m-num small lead">
						<span class="lead-dot"></span>
			...
			<section v-if="activeView === 'matrix'" class="panel glass">
				...
				<InterruptionMatrix
					v-if="visibleMatrix.length"
					:rows="visibleMatrix"
					:axis-row="i18n.t('mx_axis_applauding')"
					:axis-col="i18n.t('mx_axis_applauded')"
			...
			<section v-else-if="activeView === 'parties'" class="panel glass">
				...
				<HBars v-if="partyBars.length" :bars="partyBars" :value-label="i18n.t('bf_applause')" />
				<p v-else class="empty">—</p>
			</section>
			<section v-else class="panel glass">
				...
				<div v-if="selfVsOtherBars.length" class="svo-list">
					<div v-for="row in selfVsOtherBars" :key="row.party" class="svo-row">
						<div class="svo-label">
						...
						<div class="svo-bar-wrap">
							<div
								...
								:style="{ width: row.selfPct + '%', background: row.color }"
