...
import StoryPanel from '$lib/components/StoryPanel.vue';
import ParliamentArc from '$lib/components/ParliamentArc.vue';
import { i18n } from '$lib/i18n';

const router = useRouter();
const year = new Date().getFullYear();
const quick = ['Klimawandel', 'Digitalisierung', 'Frieden', 'Europa'];

const kw = ref('');
function search() {
	const w = kw.value.trim();
	router.push(`/explore${w ? `?word=${encodeURIComponent(w)}` : ''}`);
}

watchEffect(() => {
	document.title = `OpenBundestag — ${i18n.t('tagline')}`;
});
</script>
...
<template>
	<section class="hero">
		<div class="hero-inner wrap">
			<div class="hero-text">
				...
				<form class="search reveal d3" @submit.prevent="search">
					<input
						v-model="kw"
				...
				<div class="quick reveal d3">
					<RouterLink
						v-for="q in quick"
						:key="q"
						:to="`/explore?word=${encodeURIComponent(q)}`"
						class="quick-chip"
						>{{ q }}</RouterLink
					>
				</div>
	...
	<section class="stories wrap">
		...
		<StoryPanel v-for="(story, i) in stories" :key="story.word" :story="story" :index="i" />
	</section>
	...
	<section class="cta-band">
		<div class="wrap">
			...
			<RouterLink class="btn" to="/explore">{{ i18n.t('nav_wortsuche') }} →</RouterLink>
		</div>
	</section>
</template>
