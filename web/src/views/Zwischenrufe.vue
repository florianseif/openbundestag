...
import InterruptionMatrix from '$lib/components/InterruptionMatrix.vue';
import ZwischenrufFeed from '$lib/components/ZwischenrufFeed.vue';
...
// ── state ─────────────────────────────────────────────────────────────────
const meta = ref<ZwischenrufMeta | null>(null);
const termOptions = ref<TermInfo[]>([]);
const topCallers = ref<ZwischenrufCallerCount[]>([]);
const byParty = ref<ZwischenrufPartyCount[]>([]);
const matrix = ref<ZwischenrufMatrixRow[]>([]);
const loading = ref(true);
const bootError = ref<string | null>(null);

const terms = ref<number[]>([21]);
const activeView = ref<'matrix' | 'parties' | 'callers' | 'feed'>('matrix');

// ── politician search (matrix panel only) ─────────────────────────────────
const callerNameSearch = ref('');
const callerSamples = ref<ZwischenrufSample[]>([]);
const callerSamplesLoading = ref(false);
let callerSamplesDebounce: ReturnType<typeof setTimeout>;

watchEffect(() => {
	const name = callerNameSearch.value.trim();
	void terms.value;
	if (!name || !meta.value?.available) {
		callerSamples.value = [];
		return;
	}
	clearTimeout(callerSamplesDebounce);
	callerSamplesDebounce = setTimeout(async () => {
		callerSamplesLoading.value = true;
		try {
			callerSamples.value = await api.zwischenrufe.samples({
				callerName: name,
				terms: terms.value,
				limit: 500
			});
		} finally {
			callerSamplesLoading.value = false;
		}
	}, 350);
});

const callerTargetBars = computed(() => {
	if (!callerSamples.value.length) return [];
	const totals = new Map<string, number>();
	for (const s of callerSamples.value) {
...
const callerInfo = computed(() =>
	callerSamples.value.length > 0
		? { name: callerSamples.value[0].caller_name, party: callerSamples.value[0].caller_party }
		: null
);
...
async function boot() {
	...
	loading.value = true;
	try {
		meta.value = await api.zwischenrufe.meta();
	} catch {
		bootError.value = i18n.t('api_unreachable');
		loading.value = false;
		return;
	}
	...
	api
		.meta()
		.then((m) => (termOptions.value = m.terms))
		.catch(() => {});
	await reload();
	loading.value = false;
}
...
async function reload() {
	...
	if (tc.status === 'fulfilled') topCallers.value = tc.value;
	if (bp.status === 'fulfilled') byParty.value = bp.value;
	if (mx.status === 'fulfilled') matrix.value = mx.value;
}
...
// ── derived ───────────────────────────────────────────────────────────────
const isRealParty = (p: string | null | undefined): p is string => !!p && p !== 'Unknown';

const visibleTopCallers = computed(() => topCallers.value);
const visibleByParty = computed(() => byParty.value);
const visibleMatrix = computed(() =>
	matrix.value.filter((r) => isRealParty(r.caller_party) && isRealParty(r.target_speaker_party))
);

const callerBars = computed(() =>
	visibleTopCallers.value.map((c) => ({
		label: c.caller_name,
		sub: c.caller_party,
		value: c.n,
...
const topCaller = computed(() => visibleTopCallers.value[0] ?? null);
const topCallerParty = computed(() => visibleByParty.value[0] ?? null);
const topTarget = computed(() => {
	const totals = new Map<string, number>();
	for (const r of visibleMatrix.value) {
...
watchEffect(() => {
	document.title = `${i18n.t('zw_title')} · OpenBundestag`;
});
</script>
...
<template>
	<div class="wrap page">
		<PageHero
			:title="i18n.t('zw_title')"
			:subtitle="i18n.t('zw_subtitle')"
			:description="i18n.t('zw_explainer')"
			variant="warm"
		/>

		<div v-if="bootError" class="alert glass">
			<p>{{ bootError }}</p>
			<button class="btn" @click="boot">{{ i18n.t('retry') }}</button>
		</div>
		<div v-else-if="loading" class="boot-state">
			<div class="orb"></div>
			<p class="muted">{{ i18n.t('waking') }}</p>
		</div>
		<div v-else-if="!meta?.available" class="alert glass">
		...
		<template v-else>
			...
			<div class="metrics">
				...
				>
					...
					<span class="m-sub"
						>{{ topCaller.caller_party }} · {{ formatNumber(topCaller.n, i18n.lang) }} ×</span
					>
				...
				<div
					v-if="topCallerParty"
					class="metric glass"
					:style="{ '--lc': partyColor(topCallerParty.caller_party) }"
				>
					<span class="m-num small lead">
						<span class="lead-dot"></span>
					...
					<span class="m-sub"
						>{{ formatNumber(topCallerParty.n, i18n.lang) }} {{ i18n.t('zw_calls') }}</span
					>
				...
				<div
					v-if="topTarget"
					class="metric glass"
					:style="{ '--lc': partyColor(topTarget.party) }"
				>
					<span class="m-num small lead">
						<span class="lead-dot"></span>
						{{ topTarget.party }}
					...
					<span class="m-sub">{{
						i18n.t('zw_times_interrupted', { n: formatNumber(topTarget.n, i18n.lang) })
					}}</span>
			...
			<!-- ── Filters ────────────────────────────────────────────────────── -->
			<div v-if="termOptions.length" class="filter-bar glass">
				<TermFilter v-model:selected="terms" :options="termOptions" />
			</div>
			...
			<!-- ── View panels ────────────────────────────────────────────────── -->
			<section v-if="activeView === 'matrix'" class="panel glass">
				<header class="p-head">
					<div>
					...
					<div class="p-search-wrap">
						...
						<input
							...
							v-model="callerNameSearch"
						/>
					</div>
				</header>

				<template v-if="callerNameSearch.trim()">
					<div v-if="callerSamplesLoading" class="spotlight-loading"><span class="pulse-dot"></span></div>
					<div v-else-if="callerTargetBars.length" class="spotlight">
						<p v-if="callerInfo" class="spotlight-meta">
							<span class="spotlight-name">{{ callerInfo.name }}</span>
							<span
								v-if="callerInfo.party"
								class="spotlight-party"
								:style="{ background: partyColor(callerInfo.party) }"
								>{{ callerInfo.party }}</span
							>
							<span class="spotlight-count">{{
								i18n.t('zw_interrupted_the', { n: callerSamples.length })
							}}</span>
						</p>
						<HBars :bars="callerTargetBars" :value-label="i18n.t('zw_calls')" />
					</div>
					<p v-else class="empty">{{ i18n.t('zw_no_results') }}</p>
				</template>
				<template v-else>
					<InterruptionMatrix v-if="visibleMatrix.length" :rows="visibleMatrix" :show-samples="true" />
					<p v-else class="empty">—</p>
				</template>
			</section>
			<section v-else-if="activeView === 'parties'" class="panel glass">
				<header class="p-head"><h3>{{ i18n.t('zw_most_active_party') }}</h3></header>
				<HBars v-if="partyBars.length" :bars="partyBars" :value-label="i18n.t('zw_calls')" />
				<p v-else class="empty">—</p>
			</section>
			<section v-else-if="activeView === 'callers'" class="panel glass">
				<header class="p-head"><h3>{{ i18n.t('zw_top_title') }}</h3></header>
				<HBars v-if="callerBars.length" :bars="callerBars" :value-label="i18n.t('zw_calls')" />
				<p v-else class="empty">—</p>
			</section>
...
.adv-tab::before {
	...
	background: radial-gradient(
		...
		color-mix(in srgb, var(--tab-color) 18%, transparent),
		transparent 70%
	);
	opacity: 0;
...
.adv-tab:hover::before {
	opacity: 1;
}
.adv-tab:hover::after {
	opacity: 0.5;
}
.adv-tab:hover .adv-arrow {
	opacity: 1;
...
.adv-tab.active {
	...
	box-shadow:
		0 0 0 1px color-mix(in srgb, var(--tab-color) 30%, transparent),
		0 12px 36px -8px color-mix(in srgb, var(--tab-color) 35%, transparent);
	transform: translateY(-2px);
}
.adv-tab.active::before {
	opacity: 1;
}
.adv-tab.active::after {
	opacity: 1;
}
.adv-tab.active .adv-icon {
	color: var(--tab-color);
	filter: drop-shadow(0 0 8px color-mix(in srgb, var(--tab-color) 60%, transparent));
}
.adv-tab.active .adv-title {
	color: var(--ink);
}
.adv-tab.active .adv-arrow {
	opacity: 1;
	color: var(--tab-color);
...
@keyframes pulse {
	0% {
		box-shadow: 0 0 0 0 rgba(255, 93, 125, 0.55);
	}
	70% {
		box-shadow: 0 0 0 10px rgba(255, 93, 125, 0);
	}
	100% {
