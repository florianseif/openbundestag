...
**Drill-down (implemented):** clicking a timeline point opens `SpeechModal.svelte` — a centered glass modal listing matching speeches; each expands inline to the full text with the keyword `<mark>`-highlighted (fetched on demand via `/api/speech/{id}`) and a Download button that emits a `.txt`. Full text + paragraph snippets require raw `speech_content`; the deployed/current local DB is `--slim`, so the reader degrades gracefully to metadata + a "full text not in this build" note until a non-slim DB is shipped.
